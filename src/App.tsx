...
const App: React.FC = () => {
  ...
  const handleParse = async () => {
    ...
    if (wordCount > LARGE_TEXT_THRESHOLD) {
      toast.warning(`Large text detected (${wordCount.toLocaleString()} words). This may take a moment to process.`);
    }
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      // If confidence is low, show warning and don't auto-parse
      if (confidence < OCR_CONFIDENCE_THRESHOLD) {
        toast.warning(`OCR confidence is ${confidence.toFixed(0)}%. Please review the extracted text before starting.`);
        setIsParsing(false);
        setOcrProgress(0);
        return;
      }
          ...
          <DialogContent className="max-w-2xl bg-slate-900 border-slate-700">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="image" className="space-y-4 mt-4">
                <p className="text-sm text-slate-400">Upload an image containing text to extract it using OCR:</p>
                ...
                {ocrConfidence !== null && ocrConfidence < OCR_CONFIDENCE_THRESHOLD && (
                  <div className="space-y-3">
                    <Alert variant="destructive">
                      ...
                      <AlertDescription>
                        OCR confidence is low ({ocrConfidence.toFixed(0)}%). Please review and edit the extracted text below before starting.
                      </AlertDescription>
                    </Alert>
